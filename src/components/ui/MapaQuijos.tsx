...
import { Flame, X, MapPin, CheckCircle2, Clock } from 'lucide-react';
...
// Geografía exacta extraída de la imagen oficial del Cantón Quijos
...
type HotspotCard = { x: number; y: number; region: Region; stat: ParishReportCount; isPinned?: boolean };

export default function MapaQuijos({ data, imageSrc = '/mapa_quijos.png' }: Props) {
  const [tooltip, setTooltip] = useState<Tooltip | null>(null);
  const [hotspotCard, setHotspotCard] = useState<HotspotCard | null>(null);
  const [active, setActive] = useState<string | null>(null);

  const dataMap = Object.fromEntries(data.map((d) => [d.parish, d])) as Record<string, ParishReportCount>;
  const max = Math.max(...data.map((d) => d.total), 1);

  const moveRegion = (e: React.MouseEvent<SVGElement>, region: Region) => {
    // Si la tarjeta de punto de calor está fijada, no sobreescribir con tooltip simple
    if (hotspotCard?.isPinned) return;
    const svg = e.currentTarget.closest('svg') as SVGSVGElement;
    const rect = svg.getBoundingClientRect();
    setTooltip({ x: e.clientX - rect.left, y: e.clientY - rect.top, region, stat: dataMap[region.nombre] });
  };

  const handleHotspotTrigger = (e: React.MouseEvent<SVGElement>, region: Region, pin: boolean = false) => {
    e.stopPropagation();
    const svg = e.currentTarget.closest('svg') as SVGSVGElement;
    const rect = svg.getBoundingClientRect();
    const stat = dataMap[region.nombre];
    if (!stat) return;
  ...
  return (
    <div className="relative w-full select-none" onClick={() => { if (hotspotCard && !hotspotCard.isPinned) setHotspotCard(null); }}>
      <svg
      ...
      >
        ...
        {/* Overlay de intensidad + zonas interactivas de cada parroquia */}
        {REGIONS.map((r) => {
        ...
        {/* PUNTOS DE CALOR INTERACTIVOS (HOTSPOTS) — Despliegan la tarjeta con reporte por barrios */}
        {REGIONS.map((r) => {
          const stat = dataMap[r.nombre];
          const activeCount = (stat?.pending ?? 0) + (stat?.inProgress ?? 0);
          if (activeCount === 0) return null;
          const [cx, cy] = r.centroide;
          const isCurrentHotspot = hotspotCard?.region.id === r.id;

          return (
            <g
              key={`pulse-${r.id}`}
              transform={`translate(${cx},${cy - 55})`}
              className="cursor-pointer group"
              role="button"
              tabIndex={0}
              aria-label={`Punto de calor ${r.nombre}: ${activeCount} reportes activos`}
              onMouseEnter={(e) => handleHotspotTrigger(e, r, false)}
              onMouseMove={(e) => {
                if (!hotspotCard?.isPinned) handleHotspotTrigger(e, r, false);
              }}
              onMouseLeave={() => {
                if (!hotspotCard?.isPinned) setHotspotCard(null);
              }}
              onClick={(e) => handleHotspotTrigger(e, r, true)}
            >
              {/* Área táctil amplia e invisible (54px) para facilitar pasar el mouse o tocar en móvil */}
              <circle r="27" fill="transparent" pointerEvents="all" />

              {/* Ondas expansivas de calor */}
              <circle r="10" fill="#dc2626" opacity="0" pointerEvents="none">
              ...
              <circle r="10" fill="#dc2626" opacity="0" pointerEvents="none">
                <animate attributeName="r" values="10;45" dur="2.2s" begin="0.8s" repeatCount="indefinite" />
                <animate attributeName="opacity" values="0.6;0" dur="2.2s" begin="0.8s" repeatCount="indefinite" />
              </circle>

              {/* Núcleo sólido del punto de calor con efecto hover */}
              <circle
                r={isCurrentHotspot ? 13 : 11}
                fill="#dc2626"
                stroke="#ffffff"
                strokeWidth={2.5}
                pointerEvents="none"
                style={{
                  filter: 'drop-shadow(0 2px 6px rgba(220, 38, 38, 0.75))',
                  transition: 'r 0.2s ease',
                }}
              />
              <circle r="4" fill="#ffffff" pointerEvents="none" />
              <circle r="2" fill="#dc2626" pointerEvents="none" />
            </g>
          );
        })}

        {/* Conteo total en el centroide de cada parroquia */}
        {REGIONS.map((r) => {
          const total = dataMap[r.nombre]?.total ?? 0;
      ...
      {/* TARJETA EJECUTIVA DE PUNTO DE CALOR: DESGLOSE POR BARRIO */}
      {hotspotCard && (
        <div
          className="absolute z-50 bg-white/95 dark:bg-gray-900/95 backdrop-blur-md border border-red-200 dark:border-red-900/60 rounded-xl shadow-2xl p-4 w-72 sm:w-80 text-sm animate-in fade-in zoom-in-95 duration-150 transition-all pointer-events-auto"
          style={{
            left: Math.max(12, Math.min(hotspotCard.x - 140, 600)),
            top: Math.max(12, hotspotCard.y - 80),
          }}
          onClick={(e) => e.stopPropagation()}
        >
          {/* Header de la tarjeta */}
          <div className="flex items-start justify-between border-b border-gray-200 dark:border-gray-700/80 pb-2.5 mb-3">
              ...
              <div>
                <div className="flex items-center gap-1.5">
                  <span className="font-bold text-gray-900 dark:text-white text-sm">
                    {hotspotCard.region.nombre}
                  </span>
                  {hotspotCard.region.cabecera && (
                    <span className="text-[10px] text-emerald-700 dark:text-emerald-300 bg-emerald-50 dark:bg-emerald-950/50 border border-emerald-200 dark:border-emerald-800 rounded-full px-1.5 py-0.2">
                      Cabecera
                    </span>
                  )}
                </div>
                <p className="text-[11px] text-red-600 dark:text-red-400 font-semibold flex items-center gap-1">
                  Punto de Calor Crítico ({hotspotCard.stat.pending + hotspotCard.stat.inProgress} activos)
                </p>
              </div>
            </div>
          ...
          {/* Subtítulo de barrios */}
          <div className="mb-2 flex items-center justify-between text-xs text-gray-500 dark:text-gray-400">
            <span className="flex items-center gap-1 font-medium">
              <MapPin size={12} className="text-emerald-500" /> Reportes por Barrio / Sector
            </span>
            <span className="text-[11px] font-semibold text-gray-700 dark:text-gray-300">
              Total: {hotspotCard.stat.total}
            </span>
          </div>

          {/* Listado con barras de progreso por barrio */}
          {hotspotCard.stat.neighborhoods && hotspotCard.stat.neighborhoods.length > 0 ? (
            <div className="space-y-2 max-h-56 overflow-y-auto pr-1">
              {hotspotCard.stat.neighborhoods.map((n) => {
                const totalBarrio = n.total;
                const pctActive = totalBarrio > 0 ? (n.active / totalBarrio) * 100 : 0;
                const pctResolved = totalBarrio > 0 ? (n.resolved / totalBarrio) * 100 : 0;

                return (
                  <div
                    key={n.name}
                    className="p-2 rounded-lg bg-gray-50 dark:bg-gray-800/80 border border-gray-100 dark:border-gray-700/60"
                  >
                    <div className="flex items-center justify-between text-xs mb-1">
                      <span className="font-semibold text-gray-800 dark:text-gray-200 truncate max-w-[170px]" title={n.name}>
                        {n.name}
                      </span>
                      <span className="font-bold text-gray-900 dark:text-white">
                        {totalBarrio} {totalBarrio === 1 ? 'rep.' : 'reps.'}
                      </span>
                    </div>

                    {/* Mini barra de estado visual */}
                    <div className="w-full bg-gray-200 dark:bg-gray-700 h-1.5 rounded-full overflow-hidden flex">
                      <div
                        style={{ width: `${pctActive}%` }}
                        className="bg-red-500 h-full"
                        title={`${n.active} activos`}
                      />
                      <div
                        style={{ width: `${pctResolved}%` }}
                        className="bg-emerald-500 h-full"
                        title={`${n.resolved} resueltos`}
                      />
                    </div>
            ...
            <div className="p-3 bg-gray-50 dark:bg-gray-800/50 rounded-lg text-xs text-gray-500 dark:text-gray-400 text-center">
              Reportes registrados a nivel general de la parroquia (sin desglose específico de barrio registrado).
            </div>
          )}

          {/* Footer con resumen rápido de estados */}
          <div className="mt-3 pt-2.5 border-t border-gray-200 dark:border-gray-700/80 flex items-center justify-between text-[11px] text-gray-500 dark:text-gray-400">
            <span className="flex items-center gap-1 text-emerald-600 dark:text-emerald-400">
              <CheckCircle2 size={12} /> {hotspotCard.stat.resolved} resueltos
            </span>
            <span className="flex items-center gap-1 text-blue-600 dark:text-blue-400">
              <Clock size={12} /> {hotspotCard.stat.inProgress} en proceso
            </span>
            <span className="flex items-center gap-1 text-indigo-600 dark:text-indigo-400">
              ● {hotspotCard.stat.pending} pendientes
            </span>
          </div>
        </div>
      )}

      {/* Tooltip simple al pasar por la geografía general si no hay tarjeta de calor abierta */}
      {tooltip && !hotspotCard && (
        <div
          className="absolute z-40 pointer-events-none bg-white dark:bg-gray-800 border border-gray-200 dark:border-gray-700 rounded-lg shadow-lg px-3 py-2 text-sm"
      ...
      {/* Leyenda del Mapa */}
      <div className="absolute bottom-3 right-3 bg-white/95 dark:bg-gray-800/95 backdrop-blur-sm border border-gray-200 dark:border-gray-700 rounded-lg px-3 py-2 shadow text-xs space-y-1">
        <p className="font-semibold text-gray-600 dark:text-gray-300 mb-1">Intensidad y Alertas</p>
        <div className="flex items-center gap-2 mb-1">
          <span className="w-3 h-3 rounded-full bg-red-600 inline-block flex-shrink-0 animate-pulse" />
          <span className="text-gray-700 dark:text-gray-300 font-medium">Punto de calor (Hover / Clic)</span>
        </div>
        {[
          { o: 0.55, label: 'Muy alta' },
          { o: 0.4, label: 'Alta' },
          { o: 0.28, label: 'Media' },
          { o: 0.16, label: 'Baja' },
        ].map(({ o, label }) => (
